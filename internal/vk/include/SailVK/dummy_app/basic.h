...
/**
 * @file basic.h
 * @brief Basic Vulkan App
 * @author sailing-innocent
 * @date 2024-05-01
 */

#include "SailVK/config.h"
#include "SailGL/dummy_app/pure.h"
#include "utils.h"

namespace sail::vk {

struct SAIL_VK_API QueueFamilyIndices {
...
class SAIL_VK_API VKBasicApp : public gl::GLPureDummyApp {
public:
	VKBasicApp() = default;
	VKBasicApp(std::string _title,
			   unsigned int _resw,
			   unsigned int _resh) : gl::GLPureDummyApp(_title, _resw, _resh) {
	}
	virtual ~VKBasicApp() {
		destroy_buffers();
		destroy_window();
