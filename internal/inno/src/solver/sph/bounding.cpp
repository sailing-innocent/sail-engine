/**
 * @file bounding.cpp
 * @brief The Bounding Implementation
...
void Bounding::compile(Device& device) noexcept {
	...
	lazy_compile(device, bounding_cube, [&](Int count, Float delta_time, Float rate) {
	...
	lazy_compile(device, bounding_sphere, [&](Int count, Float delta_time, Float rate) {
		set_block_size(n_blocks);
		grid_stride_loop(count, [&](Int p) noexcept {
			Float3 x = particles.m_pos->read(p);
			Float3 v = particles.m_vel->read(p);
	...
	lazy_compile(device, bounding_waterfall, [&](Int count, Float delta_time, Float rate) {
		set_block_size(n_blocks);
		grid_stride_loop(count, [&](Int p) noexcept {
			Float3 x = particles.m_pos->read(p);
			Float3 v = particles.m_vel->read(p);
	...
	lazy_compile(device, bounding_heightmap, [&](Int count, Float delta_time, Float rate) {
		set_block_size(n_blocks);
