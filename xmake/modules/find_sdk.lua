...
function tooldir()
    return vformat("external/sdks/tools/$(host)")
end

function sdk_libdir()
    return vformat("external/sdks/libs")
end
...
function file_from_github(zip)
    local sdkdir = sdkdir or os.projectdir().."/external/sdks"
    local zip_dir = find_file(zip, {sdkdir})
    if(zip_dir == nil) then
        local url = vformat("https://github.com/SakuraEngine/Sakura.Resources/releases/download/external/sdks/")
        print("download: "..url..zip.." to: "..os.projectdir().."/external/sdks/"..zip)
        http.download(url..zip, os.projectdir().."/external/sdks/"..zip, {continue = false})
    end
end
