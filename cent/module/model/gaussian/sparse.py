...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, scaling=1.0):
        ...
        dist2 = torch.ones(N).float().cuda() * 0.001 * scaling
    ...
    def training_setup(self, training_args):
        ...
        self.xyz_scheduler_args = discrete_exp_func(
            x_init=training_args.position_lr_init*self.spatial_lr_scale, 
            x_final=training_args.position_lr_final*self.spatial_lr_scale, 
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        rots = qvec2R(self._rotation[selected_pts_mask]).repeat(N,1,1)
    ...
    def epipolar_view(self, gs2ds, cams):
        N_batch = len(cams)
        ...
        for i in range(N_batch):
            ...
            V2s.append(v[2:3,:])
            V2s.append(v[2:3,:])
            xys.append(torch.tensor(gs2ds[i].means_2d).float().cuda())
            colors.append(gs2ds[i].color_features)
            opacities.append(gs2ds[i].opacity_features)

        v01 = torch.cat(V1s, dim = 0)
        v01 = v01.unsqueeze(0).repeat(N_points, 1, 1)
        v2 = torch.cat(V2s, dim = 0)
        v2 = v2.unsqueeze(0).repeat(N_points, 1, 1)
        xy = torch.cat(xys, dim=1)
        assert xy.shape == (N_points, N_sample)
        xy = xy.reshape(N_points, N_sample, N_out_dim)
        ...
        assert W_next.shape == (N_points, N_in_dim, N_out_dim)

        self._xyz = W_next.reshape(N_points, N_in_dim)
        ...
        # self._opacity = inverse_sigmoid(torch.clamp(self._opacity, 0.01, 0.99))