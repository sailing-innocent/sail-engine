...
from utils.algorithm.rl.policy_gradient import NPG
...
def test_mujoco_invp_pg_cuda():
    ...
    for seed in [1, 2]:
        ...
        agent = NPG(obs_space_dim, act_space_dim, cuda=True)
    ...
    sns.lineplot(x="episodes", y="reward", data=df1).set(
        title="NPG for InvertedPendulum-v4"
    )
    # plt.show()
...
@pytest.mark.app
def test_mujoco_hc_npg_cuda_train():
    ...
    for seed in [42]:
        ...
        for episode in tqdm(range(total_num_episodes)):
            ...
            if (episode+1) % 1000 == 0:
                print()
                wpath = f"output/halfcheetah_npg_5e3_5en4_{episode+1}.pth"
    ...
    sns.set(style="darkgrid", context="talk", palette="rainbow")
    sns.lineplot(x="episodes", y="reward", data=df1).set(
        title="NPG for HalfCheetah-v4"
    )
    # plt.show()
    plt.savefig("output/train_halfcheetah_npg_5e3_5en4.png")
...
def test_train_nav2d():
    ...
    for seed in [42]:
        ...
        for episode in tqdm(range(total_num_episodes)):
            ...
            if (episode+1) % 100 == 0:
                print()
                wpath = f"output/nav2d_npg_5e2_5en4_{episode}.pth"
                print(f"Saved the model at {wpath}")
                agent.save(wpath)
    ...
    sns.set(style="darkgrid", context="talk", palette="rainbow")
    sns.lineplot(x="episodes", y="reward", data=df1).set(
        title="NPG for Custom Nav2d Maze"
    )
    # plt.show()
    plt.savefig("output/train_nav2d_npg_5e2_5en4.png")
...
def test_load_nav2d_pg_cuda():
    ...
    for seed in [42]:
        ...
        # reinit agent for every seed 
        agent = NPG(obs_space_dim, act_space_dim, cuda=True)
        agent.load("output/nav2d_npg_5e2_5en4_400.pth")
        reward_over_episodes = []
