\documentclass{njupre/njupre}
\title[基于可微渲染的新视角生成方法]{基于可微渲染的新视角生成方法}
\subtitle{ 开题报告 }
\author[朱子航]{\texorpdfstring{
    朱子航 \\ \smallskip 
    \textit{522022150087@smail.nju.edu.cn} \\ \smallskip
    导师：}{}
}

\date[\today]{\texorpdfstring{\today}{}}
\begin{document}
\begin{frame}
    \titlepage
\end{frame}
\begin{frame}
    \frametitle{目录}
    \tableofcontents
\end{frame}

\section{背景介绍}

\subsection{个人简介}
\input{nju_course_slide_cn}
\input{d5_intern_slide_cn}
\input{csig2023_slide_cn}

\subsection{课题背景}
\input{research_background_slide_cn}
\section{研究内容}
\subsection{神经辐射场 Neural Radiance Field (NeRF)}
\input{nerf_intro_slide_cn}
\subsection{高斯溅射 Gaussian Splatting}
\input{gaussian_intro_slide_cn}

% \section{方法介绍}
% \input{slide_method_cn}
% \section{前期成果}
% \input{slide_progress_cn}
% \section{研究计划}
% \input{slide_plan_cn}

\begin{frame}[allowframebreaks]{Reference}
    \bibliographystyle{plain}
    \bibliography{ref}
\end{frame}
\end{document}